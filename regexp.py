...
_regex_scoreboard = re.compile(
    r'\"STEAM[^"]+\"\s+\"([^"]+)\"\s+[0-9]\s+[0-9]+\s+([^\s]+)\s+(.*)'
)

_wiki_language_codes = [
    "en",
    "sv",
    "de",
    "nl",
...
re_implying = re.compile(
    r"^[&]gt[;](.*\.(gif|GIF|jpg|JPG|jpeg|JPEG|png|PNG|tiff|TIFF|bmp|BMP))\s?(\d+)?"
)

_wiki_regex = re.compile(
    "^((" + "|".join(_wiki_language_codes) + "|)\s)?(.*)$"
)  # jsem retard no, nasrat :^)
