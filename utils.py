...
from regexp import _regex_scoreboard, _named_interval_regex, _prefix_regex, _interval_regex
...
class Utils:
    ...
    def get_url(match):
        try:
            ...
            r = requests.get(match.group(1), headers=headers, allow_redirects=True, stream=True, timeout=2)
        ...
        if r.status_code != 404:
            ...
            if content_type.find("html") != -1:  # and content_type is not 'gzip':
                ...
                try:
                    ...
                    if "youtube.com" in title.lower(): # We get youtube.com seperately with video likes etc.
                        return
                    else:
                        return "<b>{}</b>".format(title)

                except: return "URL No Title ({})".format(domain)
                ...
                try:
                    ...
                    if r.headers["Content-Length"]:
                        ...
                        else: length = length = "{} bytes".format(length)
                    else:
                        length = "?"
    ...
    def get_youtube_url(match):
        ...
        return u"[YouTube] <b>{0}</b> | <strong>Views:</strong> {1:,} | <span style='color: green'>👍: {2:,}</span> | <span style='color: orange;'>👎: ?</span> ".format(video_title, int(viewCount), int(likes))
    ...
    def get_wiki_result(langcode, word):
        try:
            ...
            req = requests.get(f"https://{langcode}.wikipedia.org/w/api.php?action=opensearch&format=json&search={word}", timeout=3)
        except Exception as e:
        ...
        if parsed[1] and parsed[2] and parsed[3]:
            ...
            url = parsed[3][0]
            return "<b>{0}</b>: {1} (<a href='{2}'>{2}</a>)".format(article, shortinfo, url)
        else:
    ...
    def get_geoip(ip_addr):
        try:
            ...
            return "{0} ({1} | {2})".format(match.country, match.timezone, list(match.subdivisions)[0])
    ...
    def _parse_time(data):
        ...
        if re.match("^@.+$", data):
            ...
            if m:
                ...
                try:
                    return datetime(r["year"], r["month"], r["day"], r["hour"], r["minute"], r["second"]), r["data"]
                except:
                    return None, None
        else:
            # Time interval
            ...
            if m:
                ...
                try:
                    td = timedelta(weeks=r["w"], days=r["d"], hours=r["hr"], minutes=r["min"], seconds=r["sec"])
                    if td == 0:
                        return None, None
                    else:
                        return dt + td, r["data"]
